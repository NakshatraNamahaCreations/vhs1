...
const getenquiryadd = async () => {
  ...
  if (res.status === 200) {
    setenquiryflwdata(res.data?.quotefollowup.filter((item) => item.nxtfoll === yesterdayDate));
    setSearchResults(res.data?.quotefollowup.filter((item) => item.nxtfoll === yesterdayDate)); // Update the searchResults state with the full data
  }
};
let i = 0;
