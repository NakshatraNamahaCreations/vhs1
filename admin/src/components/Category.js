...
function Category() {
    ...
    if(!category ||!categoryImg){
        ...
        try {
          const config = {
            url: "/addcategory",
            method: "post",
            baseURL: "http://api.vijayhomeservicebengaluru.in",
            data: formdata,
          };
  ...
  const getcategory = async () => {
    let res = await axios.get("http://api.vijayhomeservicebengaluru.in/getcategory");
    if ((res.status = 200)) {
  ...
  const columns = [
    ...
    },
    {
        ...
        cell: (row) => (
          <div>
         <img src={`http://api.vijayhomeservicebengaluru.in/category/${row.categoryImg}`} width="50px" height="50px"/>
  ...
  const deletecategory = async (id) => {
    axios({
      method: "post",
      url: "http://api.vijayhomeservicebengaluru.in"+"/deletecategory/" + id,
    })
